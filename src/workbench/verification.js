...
// than a mouse. So the layout is modal — the viewer is the page, the question
// is a sheet over it, and the sheet folds away to a single line so the body is
// never hidden by the question about it. On a wide screen the same sheet is a
// rail down the right and nothing else changes.
//
...
export function mountVerificationBench(root) {
  ...
  let questions = queueDef.build();
  let idx = 0;
  let started = false;
  const decisions = {};          // question id -> answer record
  let notes = '';

  // ---- viewer tools (a bar over the view; a row in the sheet on a phone) ---
  const tools = el('div', 'vb-tools');
  const tool = (label, title, fn) => {
    const b = el('button', 'vb-tool', label);
    b.title = title;
    b.onclick = fn;
    tools.append(b);
    return b;
  };
  const FRONT = 0, BACK = Math.PI;
  tool('◀', 'Orbit left', () => { stage.cam.yaw -= 0.6; });
  tool('▶', 'Orbit right', () => { stage.cam.yaw += 0.6; });
  tool('F', 'Face on', () => stage.orbit(FRONT, 0.05));
  tool('B', 'From behind', () => stage.orbit(BACK, 0.05));
  tool('＋', 'Closer', () => { stage.cam.dist = Math.max(0.25, stage.cam.dist * 0.75); });
  tool('－', 'Further', () => { stage.cam.dist = Math.min(20, stage.cam.dist * 1.33); });
  tool('⤢', 'Whole body', () => frameBody());
  tool('◎', 'Back to the joint', () => frameQuestion(true));
  const bXray = tool('◫', 'See through the skin (wireframe)', () => {
    xray = !xray;
  ...
  // THE FOLD BAR. On a phone the sheet covers the lower half of the body, and
  // the lower half of the body is where four of the seventeen landmarks are —
  // so folding it away is part of answering, not a preference. Folded, this
  // bar is all that is left, and it still carries the question and the way
  // forward: nobody should have to unfold a sheet just to press Next.
  const fold = el('div', 'vb-fold');
  const handle = el('button', 'vb-handle', '<i></i><b></b>');
  handle.title = 'Fold the question away';
  handle.onclick = () => { card.classList.toggle('min'); requestAnimationFrame(() => frameQuestion(true)); };
  const miniNext = el('button', 'vb-mininext', 'Next ▸');
  miniNext.onclick = () => next();
  fold.append(handle, miniNext);
  ...
  card.append(fold, head, bodyBox, nav);
  wrap.append(card);

  // ---- setup screen -------------------------------------------------------
  // Everything before the first question: which file, and which fighter it is
  // standing in for. Deliberately the same three doors the other two benches
  // offer, because a model that cannot be loaded here cannot be loaded there.
  function renderSetup() {
    step.textContent = queueDef.label.toUpperCase() + ' — SETUP';
    bar.firstChild.style.width = '0%';
    bodyBox.innerHTML = '';
    nav.innerHTML = '';

    bodyBox.append(el('div', 'vb-why', queueDef.blurb));

    const sModel = sec(bodyBox, 'MODEL');
    const { box: loaderBox } = buildLoaderUI(session, {
      ...
      onLoaded: () => {
        ...
        // ball near the shoulder will point at the BALL, and the answer is
        // then the rig's own opinion handed back to it. The ◫ tool brings them
        // in when they are wanted.
        session.setSkeleton(false);
        restore(); refreshStart();
      },
      onReference: pick => {
    ...
    const sRef = sec(bodyBox, 'STANDS IN FOR');
    ...
    const go = el('button', 'vb-go', '<span>Start</span>');
    go.onclick = () => {
    ...
    const resume = el('button', 'vb-ghost', '<span>Resume</span>');
    resume.onclick = () => {
      started = true;
      idx = Math.min(questions.length - 1, Math.max(0, firstUnanswered()));
      render();
    };
    nav.append(go, resume);
    function refreshStart() {
      go.disabled = !session.model3d;
      resume.style.display = Object.keys(decisions).length ? '' : 'none';
    }
    refreshStart();
  }
  ...
  /**
   * Put the thing being asked about in the middle of the frame — and, for a
   * left/right landmark, come at it from that side, because a shoulder asked
   * about from the front is a shoulder hidden behind a chest.
   */
  function frameQuestion(keepAngle = false) {
    const q = questions[idx];
    if (!q || q.kind !== 'point') { frameBody(); return; }
    if (!keepAngle) {
    ...
    // The sheet is over the bottom of the screen on a phone rather than beside
    // the view, so BOTH the distance and the aim have to answer to it. Pulling
    // back by the covered fraction keeps the same amount of body in the part
    // that is left — a crop tight enough to hide the collar and the elbow is a
    // crop in which one patch of dark cloth is indistinguishable from another,
    // and the question stops being answerable. How much is measured, not
    // assumed, so the desktop rail (which covers nothing) changes nothing.
    const f = occlusion();
    const keep = Math.max(0.35, 1 - f);
    stage.frameOn(at, q.dist / keep);
    if (f > 0.02) {
  ...
  /** Fraction of the view's HEIGHT that the question sheet is sitting over. */
  function occlusion() {
    const v = view.getBoundingClientRect(), c = card.getBoundingClientRect();
    if (!v.height || c.left >= v.right - 1 || c.right <= v.left + 1) return 0;
    return Math.max(0, Math.min(v.bottom, c.bottom) - Math.max(v.top, c.top)) / v.height;
  }

  // ---- picking ------------------------------------------------------------
  // The view's clicks belong to the current question and to nothing else: a
  ...
  function renderBody() {
    ...
    bodyBox.innerHTML = '';
    nav.innerHTML = '';
    handle.querySelector('b').textContent = q.title;
    bodyBox.append(el('h2', 'vb-title', q.title));
    if (q.ask) bodyBox.append(el('div', 'vb-ask', q.ask));
    if (q.why) bodyBox.append(el('div', 'vb-why', q.why));
  ...
  /**
   * LEAVING A QUESTION IS WHAT RECORDS IT. There are four ways out of one — the
   * Next button, the folded bar, an arrow key, Back — and the answer has to
   * survive all of them, so the commit lives here rather than on any button. It
   * cost a mark to learn that: the fold bar advanced without it, and the export
   * came out one answer short of what was plainly on screen.
   */
  function commit() {
    const q = questions[idx];
    if (!q) return;
  ...
  function go(delta, doCommit = true) {
    if (doCommit) commit();
    idx = Math.max(0, Math.min(questions.length - 1, idx + delta));
    render();
  }
  function next() { go(1); }
  ...
  function renderPoint(q) {
    ...
    bodyBox.append(el('div', 'vb-read', read));
    ...
    other.onclick = () => { stage.cam.yaw += Math.PI; };
  ...
  function renderDone() {
    ...
    again.onclick = () => { idx = firstUnanswered(); render(); };
    const rowA = el('div', 'vb-row');
    rowA.append(apply, again);
    bodyBox.append(rowA, applyRead);
