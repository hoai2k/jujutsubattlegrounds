...
const shot = opt('--shot', null), frames = +opt('--frames', 900), script = opt('--script', 'brawl');
...
const out = await page.evaluate(async ({ p1, p2, map, frames, script, mode }) => {
  const sleep = ms => new Promise(r => setTimeout(r, ms));
  window.__skipSelect({ mode, picks: [p1, p2], map, rounds: 1 });
  const t0 = Date.now();
  ...
  while (m.tick < frames && m.phase !== 'result') { await sleep(30); tick = m.tick; const now = performance.now(); frameTimes.push(now - last); last = now; if (Date.now() - t0 > 90000) break; }
