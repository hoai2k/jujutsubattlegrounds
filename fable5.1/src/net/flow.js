...
import { NetMatch } from './sync.js';
import { watchGames } from './lobby.js';
import { getDb, availability, netError, connected } from './client.js';
import { T_FLOW, MAX_SEATS } from './config.js';
import { OnlinePanel, LobbyOverlay, NetBanner, OnlineToast } from '../ui/legacy/online.js';
import { MAPS, MAP_IDS } from '../stage/legacy/index.js';
...
export class OnlineController {
  constructor(uiRoot, sfx, input) {
    ...
    this.onStart = null;      // picks => start the match
    this.onClosed = null;     // the session ended, wherever the player is
    ...
    // fable5.1 has its own lobby screen; the old select-screen panel stays hidden
  ...
  get availability() { return this.panel.state.availability; }
  get error() { return this.panel.state.error; }
  ...
  _localSeats() {
    return Math.max(1, Math.min(MAX_SEATS, this.select?.localSeats ?? 1));
  }
  ...
  _buildMatch(payload) {
    ...
    // Hand it to the game: the front end starts the match from these picks
    // (fable5.1 has a screen router rather than a blocking select promise).
    this.select = null;
    this.onStart?.(picks);
  }
