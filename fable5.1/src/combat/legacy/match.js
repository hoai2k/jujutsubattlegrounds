...
import { resolveMelee } from './hits.js';
import { Effects } from './effects.js';
import { CPU } from './ai.js';
import { DomainSystem } from './domains.js';
import { GambleSystem } from './jackpot.js';
import { Minions } from './minion.js';
import { Judgemen } from './judgeman.js';
import { ShikigamiSystem } from './shikigami.js';
import { FloraSystem } from './flora.js';
import { SwarmSystem } from './swarm.js';
import { CurseSystem } from './curses.js';
import { FreezeSystem } from './freeze.js';
import { IceSystem } from './frost.js';
import { DomainFX } from '../../fx/legacy/domainfx.js';
import { NewShadowFx } from '../../fx/legacy/newshadowfx.js';
...
// hard concurrent cap.
import { WarpFX } from '../../fx/legacy/warpfx.js';
...
import { ConstructionSystem, tierFor as corpseTierFor } from './construction.js';
...
import { gainComedy, tierOf as comedyTierOf } from './comedy.js';
import { TheSet } from './theset.js';
import { spotlight as comedySpotlight, confetti as comedyConfetti, craftMotes, corpseDeploy, commandPulse } from '../../fx/legacy/comedyfx.js';
import { awakenBurst, massField } from '../../fx/legacy/newfx.js';
import { GarudaSystem } from './garuda.js';
import { NewShadowSystem } from './newshadow.js';
import { ReceiptSystem, canAfford } from './receipts.js';
import { BeastSystem } from './beasts.js';
import { FX as FXSystem } from '../../fx/index.js';
...
import { SpeechFX } from '../../fx/legacy/speechfx.js';
import { CommandCard } from '../../ui/legacy/commandcard.js';
import { wheelSnapshot as commandWheelSnapshot } from './speech.js';
import { BubbleSystem } from '../../fx/legacy/bubble.js';
import { tauntWeight, pickTaunt, tauntLine } from './taunts.js';
import { buildMap, applyMapLighting, registerMapGrade, currentQuality, MAP_IDS } from '../../stage/legacy/index.js';
import { FightCamera } from '../../render/camera-legacy.js';
import { seatLayer } from '../../render/stage.js';
import { setXrayFocus, clearXrayFocus } from '../../art/legacy/shaders/xray.js';
import { Ritual } from './ritual.js';
import { Finishers } from '../../finishers/index.js';
import { LegacyHUD as HUD } from '../../ui/legacy/host.js';
import { makeCharacter, pickInfo, hex } from '../../roster/index.js';
import { yawBetween, flatDist } from '../../core/math.js';
...
export class Match {
  ...
  notice(f, text) {
    ...
    // Keyed on the text so a DIFFERENT reason still gets through immediately —
    // "on cooldown" must not swallow "no mask". A wordless refusal keys on the
    // empty string, so a fizzle throttles against other fizzles and nothing
    // else.
    const key = text || '';
    if (f._noticeText === key && now - (f._noticeAt ?? -9) < 0.85) return;
    f._noticeText = key; f._noticeAt = now;
    // ---- THE FIZZLE IS ALWAYS THE ANSWER; THE WORDS ARE SOMETIMES ---------
    // Every refusal spills cursed energy, so the player always gets an
    // immediate, wordless "that did not happen" at the character. Text is
    // added only where the reason is a RULE or a RESOURCE the player has to
    // learn — no domain, need energy, the cap is two. Where the reason is a
    // timer or a state already visible on screen — a cooldown running, the
    // backlash after a domain, being in the air, a shutter already up — the
    // fizzle says everything a caption would, without putting a sentence over
    // the fight every time a button is early.
    this.fx.fizzle(f);
    this.sfx.noCE();
    if (!text) return;
  ...
  update(dt) {
    ...
    const all = this.input.frames;   // polled once per frame by the game loop
    ...
    this.fighters.forEach((f, i) => {
      ...
      if (fight) {
        if (this.net) {
          ...
          inp = (this.mode === 'cpu' && i === 1) ? (this.cpu ? this.cpu.frame() : (this.dummyInput?.() || null)) : (frames[i] || null);
        }
    ...
    for (const e of f.events) {
      switch (e.type) {
        ...
        case 'constructTier':
          ...
          break;
        case 'constructStalled': this.notice(f, 'NEED CURSED ENERGY'); break;
        ...
        case 'constructCapped':
          this.notice(f, 'MAX CONSTRUCTS: ' + (f.cfg.special?.maxCorpses ?? 2));
          break;
        ...
        // ---- DAGON --------------------------------------------------------
        case 'summonCapped':
          this.notice(f, 'MAX SUMMONS: ' + (e.max ?? 1) + ' OUTSIDE THE DOMAIN');
          break;
        case 'overheatStart':
        ...
        case 'fingerStart':
          ...
          this.cam.shake(0.25);
          break;
        case 'noFingers': this.notice(f, 'NO FINGERS LEFT'); break;
        ...
        case 'fireArrowCancel':
          ...
          break;
        // ---- HIGURUMA -----------------------------------------------------
        case 'judgemanAlive': this.notice(f, ''); break;   // he is stood on the field
        case 'confiscated': {
        ...
        case 'redScaleEnd':
          ...
          break;
        case 'redScaleUp': this.notice(f, ''); break;   // the boil is all over him
        case 'piercingLoad':
          ...
          break;
        case 'noBlood':
          this.notice(f, 'NEED BLOOD ' + Math.round(e.have) + '/' + Math.round(e.need));
          break;
        ...
        case 'essence':
          ...
          break;
        case 'noEssence':
          this.notice(f, 'NEED ESSENCE ' + Math.round(e.have) + '/' + Math.round(e.need));
          break;
        ...
        // `??`, NOT `||`. An empty `why` is a deliberate instruction — refuse
        // wordlessly, the fizzle is enough — and `||` would read it as absent
        // and put the default sentence back.
        case 'noCE': this.notice(f, e?.why ?? 'NEED CURSED ENERGY'); break;
        case 'onCooldown': this.notice(f, ''); break;   // a timer, and the fizzle says it
        case 'swordPickup':
        ...
        case 'awakenGate':
          this.notice(f, e.used ? 'AWAKENING ALREADY USED'
            : 'AWAKENING ' + e.stage + '/' + e.need);
          break;
        // ---- yuki: star rage ----------------------------------------------
        ...
        case 'stanceTier':
          ...
          break;
        case 'needStance': this.notice(f, 'NEEDS THE STANCE'); break;
        case 'ctSealed': this.notice(f, 'TECHNIQUES SEALED'); break;
        case 'nullifyCooling': this.notice(f, ''); break;   // a cooldown
        case 'nullifyLand':
        ...
        case 'assassinCooling':
          this.notice(f, '');   // a cooldown, and the fizzle says it
          break;
        case 'assassinWhiff':
        ...
        case 'chargeTier':
          ...
          break;
        case 'noCharge': this.notice(f, 'NEED CHARGE'); break;
        case 'arcSpent': this.notice(f, 'ARC DASH SPENT'); break;
        case 'arcDash':
          this.cam.shake(0.2);
          break;
        case 'staggered':
        ...
        // different answers.
        case 'noStock':
          this.notice(f, 'NO STOCK — ¥'
            + Math.round(e.need * f.cfg.receipts.yenPerStock).toLocaleString('en-US') + ' SHORT');
          break;
        case 'receiptsWet':
        ...
        case 'utterBroken':
          ...
          break;
        case 'silenced': this.notice(f, 'NO VOICE'); break;
        case 'throatTooHigh':
