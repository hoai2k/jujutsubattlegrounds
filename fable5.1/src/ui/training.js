...
export class Training {
  constructor(root, match, input, sfx) {
    ...
    this.cpu = match.cpus.get(1); match.cpus.delete(1);
  ...
  update(dt, frame) {
    ...
    // dummy input
    if (this.mode === 2) { if (!m.cpus.has(1)) m.cpus.set(1, this.cpu); } else m.cpus.delete(1);
    if (this.mode === 1 && d.state !== 'guardBreak') { const f = m.inputs.get(d) || {}; d._forceBlock = true; if (['idle', 'walk', 'run'].includes(d.state)) d.setState('block', { clip: 'block' }); }
    if (this.mode === 1 && d.state === 'block') d.f = 20;
    // refills
    for (const f of m.fighters) { if (f.comboTimer > 2 && f.state !== 'hitLight' && f.state !== 'hitHeavy' && !['launched', 'knockdown', 'getup'].includes(f.state)) { f.res.hp = Math.max(f.res.hp, f.maxHP * 0.999) ; } if (f.comboTimer > 2) { f.res.curCE = Math.max(f.res.curCE, f.res.maxCE); f.res.stamina = f.cfg.stats.stamina; if (f.res.maxCE < 100) f.res.maxCE = Math.min(100, f.res.maxCE + dt * 8); } }
  ...
  reset() { const m = this.match; m.fighters.forEach((f, i) => { const sp = m.arena.spawnPoint(i, 2); f.pos.copy(sp); f.vel.set(0, 0, 0); f.facing = i === 0 ? Math.PI / 2 : -Math.PI / 2; f.setState('idle', { clip: 'idle' }); f.res.hp = f.maxHP; f.juggle = 0; }); m.effects.clear(); this.sfx.uiSwoosh(); }
